...
describe("extractCoordinates", () => {
  ...
  it('JSON オブジェクトの引用符付きラベル ("lat": ..., "lon": ...) を抽出する', () => {
    expect(
      extractCoordinates(
        '{"latlons":[{"lat":35.668839215551216,"lon":140.00475683022427},{"lat":35.669421791182614,"lon":140.00529804754848}]}'
      )
    ).toEqual([
      [35.668839215551216, 140.00475683022427],
      [35.669421791182614, 140.00529804754848],
    ]);
  });

  it("エスケープされた JSON 文字列内の lat/lon を抽出する", () => {
    expect(extractCoordinates(readFixture("labeled-json-escaped.txt"))).toEqual([
      [35.668839215551216, 140.00475683022427],
      [35.669421791182614, 140.00529804754848],
      [35.669724956832596, 140.004971739671],
      [35.669728689014796, 140.00496772126823],
      [35.669729757349444, 140.00496657039747],
      [35.66926240905617, 140.00432061451252],
      [35.66928073031869, 140.00430173189477],
      [35.669266213184045, 140.00431669573956],
      [35.668839215551216, 140.00475683022427],
    ]);
  });
